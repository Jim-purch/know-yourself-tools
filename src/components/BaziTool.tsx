    ...
    const Pillar = ({ name, stem, branch }: { name: string, stem: string, branch: string }) => (
        <div className="flex flex-col items-center gap-3">
            ...
            <div className="flex flex-col gap-2">
                <div className="w-16 h-16 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center text-2xl font-bold text-indigo-400 shadow-inner">
                    {stem}
                </div>
                <div className="w-16 h-16 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center text-2xl font-bold text-emerald-400 shadow-inner">
                    {branch}
                </div>
            </div>
        </div>
    ...
    return (
        <div className="p-8 max-w-4xl mx-auto">
            <header className="mb-12">
                <h2 className="text-3xl font-bold mb-2">八字排盘</h2>
                <p className="text-slate-400">输入您的生辰信息，探索命学奥秘。</p>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                ...
                <div className="md:col-span-2 glass p-8 min-h-[300px] flex items-center justify-center relative overflow-hidden">
                    {!result ? (
                        <p className="text-slate-500 italic">请在左侧输入信息后点击排盘</p>
                    ) : (
                        <motion.div
                    ...
                    <div className="absolute bottom-0 left-0 w-32 h-32 bg-emerald-500/5 rounded-full -ml-16 -mb-16 blur-3xl"></div>
                </div>
            </div>
            ...
            {result && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-slate-800/30 p-6 rounded-xl border border-slate-700">
                        <h4 className="font-bold mb-4 text-indigo-400">排盘简析</h4>
                        <div className="space-y-3 text-slate-300 text-sm leading-relaxed">
                            <p>日主属：<span className="text-white font-bold">{result.day.stem}</span>，代表您的核心原色。</p>
